...
class OrmTaskRepository:
    @staticmethod
    def create_task(user_id: int, name: str, text: str, state: str, folder_id: int) -> Task | None:
        session = db.get_session()
        try:
            task = Task(user_id=user_id, name=name, text=text, state=state, date=datetime.now(timezone.utc), folder_id=folder_id)
            session.add(task)
            session.commit()
    ...
    @staticmethod
    def update_task(user_id: int, task_id: int, name: str, text: str, state: str, folder_id: int) -> Task | None:
        session = db.get_session()
        try:
