...
```bash
# После изменения моделей в tasks_api/database/models.py
...
3. **Проверьте миграции**: Убедитесь, что миграции применены (`alembic current`)
4. **Проверьте параметры подключения**: Убедитесь, что в `.env` указаны корректные параметры
5. **Проверьте токен**: Убедитесь, что JWT токен валиден и не просрочен
6. **Проверьте SQLAlchemy соединение**: Установите `DB_ECHO=true` для отладки SQL запросов
